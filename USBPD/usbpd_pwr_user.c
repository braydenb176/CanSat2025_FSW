...
__weak int32_t BSP_USBPD_PWR_VBUSGetVoltage(uint32_t Instance, uint32_t *pVoltage)
{
  ...
  else
  {
	...
	voltage = (uint32_t) __LL_ADC_CALC_DATA_TO_VOLTAGE(3300u, (uint32_t) LL_ADC_REG_ReadConversionData12(hadc1.Instance), LL_ADC_RESOLUTION_12B); /* mV */

	/* (Divider 10k/100k for VSENSE */
	voltage *= 10989u;
	voltage /= 1000u;
